...
| [BLACKLIST_API.md](BLACKLIST_API.md) | **Frontend admin contract** for the image blacklist — create/list/patch/delete entries + attach/remove references + backfill, **and the new GPU-free `/cross-reference`** (does an entry appear in indexed runs? → [IMAGE_INDEX_API.md §6](apis/IMAGE_INDEX_API.md)). Now documents the **gateway base path** `/api/v1/images/blacklist`. | wiki |
...
| [image-index/02_SEARCH_DESIGN.md](image-index/02_SEARCH_DESIGN.md) | **Validated design (SHIP)** for two additions: **(A)** async search-by-image scoped to `external_ids` (reuses the evidence `search_requests`/`search_matches` + compute embed round-trip, retargeted to `image_index_embeddings`); **(B)** GPU-free blacklist cross-reference (stored-vector search, no compute). Cross-collection same-space proof, IDOR, non-regression, phased plan (P1 qdrant primitives → P2 A → P3 B REST → P3b B auto-hook v1.1 → P4 e2e). | wiki (design) |
